...
class AVNewFeedFiveImageTableViewCell: AVBaseNewFeedTableViewCell {
    ...
    override func configCell(data: DataForItem, host: String,vc:UIViewController) {
        ...
        self.img3.sd_setImage(with: URL(string: host + data.post.image[2].urlImage), placeholderImage: UIImage(named: "PlaceHolder.png"), options: []) { (someImage, someError, type, urlImage) in
            var photo:SKPhoto?
            if someImage != nil {
                photo = SKPhoto.photoWithImage(someImage!)
            } else {
        ...
        self.img4.sd_setImage(with: URL(string: host + data.post.image[3].urlImage), placeholderImage: UIImage(named: "PlaceHolder.png"), options: []) { (someImage, someError, type, urlImage) in
            var photo:SKPhoto?
            if someImage != nil {
                photo = SKPhoto.photoWithImage(someImage!)
            } else {
        ...
        self.img5.sd_setImage(with: URL(string: host + data.post.image[4].urlImage), placeholderImage: UIImage(named: "PlaceHolder.png"), options: []) { (someImage, someError, type, urlImage) in
            var photo:SKPhoto?
            if someImage != nil {
                photo = SKPhoto.photoWithImage(someImage!)
            } else {
